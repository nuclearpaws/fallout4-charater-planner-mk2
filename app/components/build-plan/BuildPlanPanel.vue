...
import { lateLevelsForPlanAction, type PlanAction } from '~/utils/planner'
...
<template>
  <section class="tab-panel plan-panel">
    ...
    <div class="plan-workspace">
      ...
      <section class="plan"><h2>Optimized route</h2><p class="hint">Priorities are honored where game requirements allow.</p><ol v-if="plan.length" class="plan-list"><li v-for="(action, index) in plan" :key="`${action.level}-${action.label}-${index}`" :class="[action.type, routeTone(action)]"><b>{{ action.type === 'bobblehead' || action.type === 'book' ? `Before level ${action.level}` : `Level ${action.level}` }}</b><span>{{ action.label }}</span><small v-if="action.requiredLevel" class="plan-requirement">Requires level {{ action.requiredLevel }}<em v-if="lateLevels(action)">+{{ lateLevels(action) }} levels late</em></small><small>{{ action.detail }}</small></li></ol><p v-else-if="planError" class="error">{{ planError }}</p><p v-else class="empty">Choose perk ranks to create a route.</p></section>
    </div>
  </section>
</template>
